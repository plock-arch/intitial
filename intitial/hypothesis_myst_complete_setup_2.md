...
### Step 1.3: Modify Dockerfile for JFrog Proxy

**CRITICAL STEP**: The Dockerfile itself tries to pull images from Docker Hub, which is blocked. We need to modify it first.

```bash
# Backup the original Dockerfile
cp Dockerfile Dockerfile.original

# Automatically replace all FROM statements to use JFrog proxy
sed -i 's|FROM python:|FROM dfa.jfrog.io/docker-proxy/python:|g' Dockerfile
sed -i 's|FROM node:|FROM dfa.jfrog.io/docker-proxy/node:|g' Dockerfile
sed -i 's|FROM \([^/]*\):|FROM dfa.jfrog.io/docker-proxy/\1:|g' Dockerfile

# Verify the changes worked
echo "=== Checking Dockerfile modifications ==="
grep "^FROM" Dockerfile
```

**Expected output:**
```
FROM dfa.jfrog.io/docker-proxy/python:3.11.11-alpine3.19
FROM dfa.jfrog.io/docker-proxy/node:25.2-alpine
```

**Why this is needed:** Your company blocks direct access to Docker Hub (docker.io). Every image, even inside the Dockerfile build process, must come through JFrog proxy.

### Step 1.4: Modify docker-compose.yml for JFrog Proxy

Now modify the docker-compose.yml to use JFrog for supporting services.
...
### Step 1.5: Build the Hypothesis Image Locally
...
### Step 1.6: Update docker-compose.yml to Use Your Built Image
...
### Issue: "Access to registry-1.docker.io has been restricted"

**This is THE most common issue!**

**Problem:** The Dockerfile is trying to pull images from Docker Hub during build.
...
# Fix the Dockerfile to use JFrog proxy
sed -i 's|FROM python:|FROM dfa.jfrog.io/docker-proxy/python:|g' Dockerfile
sed -i 's|FROM node:|FROM dfa.jfrog.io/docker-proxy/node:|g' Dockerfile
sed -i 's|FROM \([^/]*\):|FROM dfa.jfrog.io/docker-proxy/\1:|g' Dockerfile
...
**Why this happens:** Your company blocks direct Docker Hub access. EVERY image reference, even inside Dockerfiles, must use the JFrog proxy path.
...
# Fix Dockerfile for JFrog (if getting docker.io errors)
cd ~/h
sed -i 's|FROM \([^/]*\):|FROM dfa.jfrog.io/docker-proxy/\1:|g' Dockerfile
...
- [ ] **Modified Dockerfile to use JFrog proxy** (CRITICAL!)
